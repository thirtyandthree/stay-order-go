...
func FetchProducts() ([]Product, error) {
	rows, err := db.Query("SELECT id, name, price, imageUrl, sizes, temperatures, addons, stock, category FROM cheapgoods")
	if err != nil {
...
func AddProduct(p Product) error {
	...
	if err != nil {
		fmt.Println("Error marshalling sizes:", err)
		return err
	}

	temperatures, err := json.Marshal(p.Temperatures)
	if err != nil {
		fmt.Println("Error marshalling temperatures:", err)
		return err
	}

	addons, err := json.Marshal(p.Addons)
	if err != nil {
		fmt.Println("Error marshalling addons:", err)
		return err
	}
	...
	stmt, err := db.Prepare("INSERT INTO cheapgoods(id, name, price, imageUrl, sizes, temperatures, addons, stock, category) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)")
	if err != nil {
...
func UpdateProduct(p Product) error {
	...
	stmt, err := db.Prepare("UPDATE cheapgoods SET name=?, price=?, imageUrl=?, sizes=?, temperatures=?, addons=?, stock=?, category=? WHERE id=?")
	if err != nil {
		return err
	}
	defer stmt.Close()
...
func GetMaxID() (int, error) {
	...
	err := db.QueryRow("SELECT MAX(id) FROM cheapgoods").Scan(&maxID)
	if err != nil {
